...
public:
	static ClipCandidate buildForRange(const TranscriptIndex &index, const CandidateGenerationOptions &generation,
	...
	static ClipCandidate
	buildVariantFromSeed(const TranscriptIndex &index, const CandidateGenerationOptions &generation,
			     const CheapScoringContext &scoring, const CandidateQualityGateOptions &qualityGate,
			     const ClipCandidate &seed, const ClipDuration &range, const QString &variantEvidence);
	static ClipCandidate scoreStructurally(const TranscriptIndex &index, const ClipCandidate &candidate);
	static bool isStructurallyViable(const ClipCandidate &candidate, const CandidateGenerationOptions &generation,
