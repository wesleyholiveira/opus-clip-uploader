...
double expensiveStagePriority(const ClipCandidate &candidate)
{
	...
	const double semanticPositive =
		std::max({candidate.scores.semanticTarget, candidate.scores.semanticClipValue,
			  candidate.scores.semanticViewerMessage, candidate.scores.semanticDirectAnswer,
			  candidate.scores.semanticEmpathy, candidate.scores.semanticResolution,
			  candidate.scores.semanticOpeningHook, candidate.scores.semanticEndingResolution});
	const double rerankerSignal = candidate.rerankerAttempted ? candidate.scores.rerankerRaw
								  : candidate.scores.reranker;
...
QVector<ClipCandidate> limit(QVector<ClipCandidate> candidates, int maxCandidates, const QString &evidence)
{
	...
	for (ClipCandidate &candidate : candidates) {
		...
		candidate.evidence.append(QStringLiteral("expensive_stage_priority:%1")
						  .arg(QString::number(expensiveStagePriority(candidate), 'f', 2)));
	}
	return candidates;
}
