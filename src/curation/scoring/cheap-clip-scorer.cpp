...
{
	return Curation::emotionalScoreForText(text) >= 0.20 || Curation::adviceScoreForText(text) >= 0.20 ||
...
bool CheapClipScorer::looksLikeQuestionOrViewerMessage(const QString &text) const
{
	...
	const bool explicitQuestion = lower.contains(QLatin1Char('?')) ||
		containsAny(lower, {QStringLiteral("como eu"), QStringLiteral("como posso"), QStringLiteral("por que"),
	...
	const bool viewerCue = Curation::textHasViewerExchangeSignals(text) ||
		containsAny(lower, {QStringLiteral("mandou"), QStringLiteral("perguntou"), QStringLiteral("comentou"),
	...
	return explicitQuestion || viewerCue;
}
...
double CheapClipScorer::finalScore(const ClipCandidate &candidate, const CheapScoringContext &context) const
{
	...
	if (context.reliableMainTarget && !context.mainTarget.trimmed().isEmpty()) {
		return boundedScore((s.semanticTarget * 0.42) + (s.viewerResponse * 0.24) + (s.boundary * 0.18) +
				    (s.topicContinuity * 0.10) + (s.emotional * 0.06));
	}
	...
	return boundedScore((s.boundary * 0.20) + (s.duration * 0.15) + (s.hook * 0.12) +
			    ...
			    (s.topicContinuity * 0.19));
}

QStringList CheapClipScorer::targetTerms(const QString &target) const
{
	QStringList words = normalizedText(target).split(QLatin1Char(' '), Qt::SkipEmptyParts);
	words.erase(std::remove_if(words.begin(), words.end(), [](const QString &word) {
		static const QSet<QString> stopWords{QStringLiteral("a"), QStringLiteral("an"), QStringLiteral("the"),
					       ...
					       QStringLiteral("their"), QStringLiteral("single"), QStringLiteral("viewer"),
					       QStringLiteral("message"), QStringLiteral("response"), QStringLiteral("uma"),
					       QStringLiteral("um"), QStringLiteral("de"), QStringLiteral("da"),
					       QStringLiteral("do"), QStringLiteral("para"), QStringLiteral("com")};
