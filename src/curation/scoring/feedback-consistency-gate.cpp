...
bool FeedbackConsistencyGate::shouldReject(const ClipCandidate &candidate,
	const FeedbackSimilarityFeatures &features, const FeedbackConsistencyGateOptions &options) const
{
	...
	if (features.negativeRangeContamination && !features.explainedByPositiveRange)
		return true;
	if (features.negativeScore >= features.positiveScore + options.hardNegativeMargin && features.negativeScore >= 0.46)
		return true;

	if (options.viewerMessagePreset) {
		const bool hasUsefulFeedbackSupport = features.positiveScore >= 0.38 && features.margin >= options.minPositiveMargin;
		...
		const bool hasSemanticSupport = std::max({candidate.scores.semanticClipValue, candidate.scores.semanticDirectAnswer,
			candidate.scores.semanticViewerMessage, candidate.scores.semanticTarget, candidate.scores.viewerResponse}) >= 0.58;
...
void FeedbackConsistencyGate::applyPositiveBoost(ClipCandidate &candidate,
	const FeedbackSimilarityFeatures &features, const FeedbackConsistencyGateOptions &options) const
{
	...
	if (features.positiveScore >= 0.32) {
		...
		candidate.scores.qualityGate = std::max(candidate.scores.qualityGate, 0.42 + (features.positiveScore * 0.32));
		candidate.evidence.append(QStringLiteral("feedback_positive_guided_score:%1").arg(QString::number(features.positiveScore, 'f', 2)));
		if (!features.positiveReason.trimmed().isEmpty())
			candidate.evidence.append(QStringLiteral("feedback_positive_reason:%1").arg(features.positiveReason.left(96)));
	}
	if (features.negativeScore >= 0.28) {
		candidate.scores.final = boundedScore(candidate.scores.final - (features.negativeScore * 0.10));
		candidate.evidence.append(QStringLiteral("feedback_negative_guided_score:%1").arg(QString::number(features.negativeScore, 'f', 2)));
		if (!features.negativeReason.trimmed().isEmpty())
			candidate.evidence.append(QStringLiteral("feedback_negative_reason:%1").arg(features.negativeReason.left(96)));
	}
	if (options.viewerMessagePreset && features.positiveScore >= 0.46 && features.margin >= options.minPositiveMargin) {
		candidate.scores.semanticClipValue = std::max(candidate.scores.semanticClipValue, 0.64 + features.positiveScore * 0.18);
		candidate.scores.semanticTarget = std::max(candidate.scores.semanticTarget, 0.60 + features.positiveScore * 0.16);
		candidate.evidence.append(QStringLiteral("feedback_consistency_positive_margin"));
	}
}

QVector<ClipCandidate> FeedbackConsistencyGate::apply(QVector<ClipCandidate> candidates,
	...
	for (ClipCandidate &candidate : candidates) {
		...
		if (!shouldReject(candidate, features, options)) {
			if (isUserAcceptedSeed(candidate)) {
				...
				candidate.evidence.append(QStringLiteral("feedback_positive_seed_kept_as_ground_truth"));
			}
