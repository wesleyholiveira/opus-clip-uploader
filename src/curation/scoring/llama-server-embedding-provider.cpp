...
SemanticEmbedding LlamaServerEmbeddingProvider::embed(const QString &text) const
{
	...
	if (error != QNetworkReply::NoError) {
		markFailure(QStringLiteral("llama_server_http_error:%1").arg(errorString));
		return {};
	}

	SemanticEmbedding embedding = parseEmbeddingResponse(response);
	if (!embedding.isValid()) {
		markFailure(QStringLiteral("llama_server_invalid_embedding_response"));
		return {};
	}
...
void LlamaServerEmbeddingProvider::markFailure(const QString &message) const
{
	failed_ = true;
	lastError_ = message;
}
