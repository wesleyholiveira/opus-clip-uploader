...
static bool hasCandidateLevelDefectCollapse(const ClipCandidate &candidate, double rawScore, double rawOpeningDefect,
					    double rawEndingDefect, double rawStructureDefect)
{
	...
	const double semanticPositive = std::max(
		{candidate.scores.semanticClipValue, candidate.scores.semanticEmpathy, candidate.scores.semanticHook,
	...
	const bool socialMetaDominated =
		!empathyBacked &&
						    ...
						    const SemanticReranker *reranker) const
{
	...
	const bool hasValidSemanticEmbeddings =
		std::any_of(candidates.constBegin(), candidates.constEnd(),
	...
	const QVector<double> rawOpeningDefects =
		scoreBatchChunked(reranker, openingDefectQueryForContext(context), candidateTexts);
	const QVector<double> rawEndingDefects =
		scoreBatchChunked(reranker, endingDefectQueryForContext(context), candidateTexts);
	const QVector<double> rawStructureDefects =
		scoreBatchChunked(reranker, structureDefectQueryForContext(context), candidateTexts);
	...
	const QVector<double> absoluteOpeningDefects = hasOpeningDefects ? absoluteScores(rawOpeningDefects)
									 : QVector<double>();
	const QVector<double> absoluteEndingDefects = hasEndingDefects ? absoluteScores(rawEndingDefects)
								       : QVector<double>();
	const QVector<double> absoluteStructureDefects = hasStructureDefects ? absoluteScores(rawStructureDefects)
									     : QVector<double>();
	const QVector<double> relativeOpeningDefects = hasOpeningDefects ? normalizedScores(absoluteOpeningDefects)
									 : QVector<double>();
	const QVector<double> relativeEndingDefects = hasEndingDefects ? normalizedScores(absoluteEndingDefects)
								       : QVector<double>();
	const QVector<double> relativeStructureDefects =
		hasStructureDefects ? normalizedScores(absoluteStructureDefects) : QVector<double>();
	...
	const bool anyDefectBatchSaturated = openingDefectsSaturated || endingDefectsSaturated ||
					     structureDefectsSaturated;
	...
	for (qsizetype i = 0; i < candidates.size(); ++i) {
		...
		const double rawStructureDefect = hasStructureDefects ? scoreAtOrZero(absoluteStructureDefects, i)
								      : 0.0;
		double openingDefect = hasOpeningDefects
					       ? calibratedDefectScore(absoluteOpeningDefects, relativeOpeningDefects,
								       i, openingDefectsSaturated)
					       : 0.0;
		double endingDefect = hasEndingDefects
					      ? calibratedDefectScore(absoluteEndingDefects, relativeEndingDefects, i,
								      endingDefectsSaturated)
					      : 0.0;
		double structureDefect = hasStructureDefects ? calibratedDefectScore(absoluteStructureDefects,
										     relativeStructureDefects, i,
										     structureDefectsSaturated)
							     : 0.0;
		const bool candidateDefectCollapse = hasCandidateLevelDefectCollapse(
			candidate, rawScore, rawOpeningDefect, rawEndingDefect, rawStructureDefect);
		...
		const double rerankerScore =
			rerankerScoreFromRaw(rawScore, boundedScore(normalizedPositiveScores.at(i)), defectScore);
		...
		candidate.evidence.append(QStringLiteral("reranker_language:%1")
						  .arg(normalizedSemanticLanguageCode(context.transcriptionLanguage,
										      context.sourceLanguage)));
		candidate.evidence.append(QStringLiteral("reranker_raw:%1").arg(QString::number(rawScore, 'f', 2)));
		candidate.evidence.append(
			QStringLiteral("reranker_opening_defect:%1").arg(QString::number(openingDefect, 'f', 2)));
		candidate.evidence.append(
			QStringLiteral("reranker_ending_defect:%1").arg(QString::number(endingDefect, 'f', 2)));
		candidate.evidence.append(
			QStringLiteral("reranker_structure_defect:%1").arg(QString::number(structureDefect, 'f', 2)));
		candidate.evidence.append(
			QStringLiteral("reranker_defect:%1").arg(QString::number(defectScore, 'f', 2)));
		if (anyDefectBatchSaturated) {
			...
			candidate.evidence.append(QStringLiteral("reranker_raw_defects:%1/%2/%3")
							  .arg(QString::number(rawOpeningDefect, 'f', 2),
							       QString::number(rawEndingDefect, 'f', 2),
...
QString SemanticRerankerStage::queryForContext(const SemanticRerankerContext &context) const
{
	...
	if (isPortugueseSemanticLanguage(languageCode)) {
		task = semanticLanguageInstruction(languageCode) +
		       QStringLiteral(
		...
		if (!target.isEmpty())
			task += QStringLiteral(
					"O melhor trecho deve ser especificamente sobre: %1. Penalize fortemente respostas genéricas de viewer, moderação, jogos, meta-chat, agradecimentos ou conversa casual que não permaneçam nesse tema do começo ao fim. ")
					.arg(target);
		else if (context.presetId == QStringLiteral("viewer_message_response"))
			task += QStringLiteral(
				"Prefira uma resposta completa e valiosa para uma única pergunta/mensagem do chat; aceite também um momento autossuficiente que gere curiosidade, empatia, conselho amoroso/emocional, reflexão ou história, desenvolva a ideia e chegue a um payoff local mesmo sem Q&A explícito. Penalize ranges dominados por agradecimento de presente/item virtual/social, conversa casual, estado físico ou curiosidade trivial sem arco útil. ");
	...
	if (!target.isEmpty())
		task += QStringLiteral(
				"The best excerpt must be specifically about: %1. Strongly penalize generic viewer answers, moderation, games, meta-chat, thanks, or casual talk that does not stay on that topic from start to finish. ")
				.arg(target);
	else if (context.presetId == QStringLiteral("viewer_message_response"))
		task += QStringLiteral(
			"Prefer one complete valuable answer to a single chat question/message; also accept a self-contained curiosity/empathy/relationship advice/story/opinion moment with development and local payoff even when it is not explicit Q&A. Penalize ranges dominated by virtual gift thanks/social/meta, casual check-ins, body status, or trivial one-off answers without a useful arc. ");
	return task.simplified();
}
...
QString SemanticRerankerStage::openingDefectQueryForContext(const SemanticRerankerContext &context) const
{
	const QString languageCode =
		normalizedSemanticLanguageCode(context.transcriptionLanguage, context.sourceLanguage);
	if (isPortugueseSemanticLanguage(languageCode)) {
		return (semanticLanguageInstruction(languageCode) +
			QStringLiteral(
				...
				"Nota baixa quando o início já contém a questão, o contexto essencial ou a primeira frase autossuficiente da resposta."))
			.simplified();
	}
	return (semanticLanguageInstruction(languageCode) +
		QStringLiteral(
	...
	if (isPortugueseSemanticLanguage(languageCode)) {
		return (semanticLanguageInstruction(languageCode) +
			QStringLiteral(
				...
				"Nota baixa quando termina na resolução natural da mesma troca ou contém apenas palavras finais necessárias."))
			.simplified();
	}
	return (semanticLanguageInstruction(languageCode) +
		QStringLiteral(
			...
			"a pause followed by a new question/vocative, another chat comment, technical troubleshooting, social/meta, a rant/sermon or different topic after the payoff. Give a low score when it ends at the natural resolution or only includes necessary closing words."))
		.simplified();
}
	...
	if (isPortugueseSemanticLanguage(languageCode)) {
		return (semanticLanguageInstruction(languageCode) +
			QStringLiteral(
				...
				"Nota baixa significa arco único aceitável com abertura, desenvolvimento e conclusão local, inclusive história, opinião ou momento de curiosidade sem Q&A explícito."))
			.simplified();
	}
	return (semanticLanguageInstruction(languageCode) +
		QStringLiteral(
						    ...
						    const SemanticRerankerContext &context) const
{
	...
	const QString languageCode =
		normalizedSemanticLanguageCode(context.transcriptionLanguage, context.sourceLanguage);
	const QString transcript = candidate.timedText.trimmed().isEmpty() ? candidate.text.simplified()
									   : candidate.timedText.simplified();
	QString document =
		QStringLiteral(
			"%1Duration: %2s. Pausa antes: %3s. Pausa depois: %4s. Maior pausa interna: %5s. Transcript with timestamps and pauses: %6")
