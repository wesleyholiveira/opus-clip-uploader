    ...
    QObject::connect(thread, &QThread::finished, context,
                     [safeContext, safeProgress, transcriptResult, cancelRequested, operationFinished,
                      finishedCallback = std::move(finishedCallback)]() mutable {
                         ...
                         if (finishedCallback)
                             finishedCallback(transcriptResult ? *transcriptResult : RecordingTranscript{}, canceled);
                     });
    thread->start();
}

static void transcribe_video_with_progress_dialog_after_cache_async(QWidget *parent, const QString &videoPath,
                                                                   const QString &transcriptionLanguage,
                                                                   std::function<void(RecordingTranscript, bool)> finishedCallback);
                                         ...
                                         std::function<void(RecordingTranscript)> finishedCallback)
{
    ...
    auto *thread = QThread::create([videoPath, normalizedLanguage, transcriptResult]() {
        ...
        const QString cacheKey = whisperXPrimary
        ...
        if (transcriptResult) {
            *transcriptResult = whisperXPrimary
        ...
        [parent, videoPath, normalizedLanguage, callback](RecordingTranscript cached) mutable {
            if (!cached.segments.isEmpty()) {
                ...
                blog(LOG_INFO,
                     ...
                     whisperXPrimary ? "whisperx_primary" : "default", cached.hasWordTimings() ? "true" : "false");
                                                                   ...
                                                                   std::function<void(RecordingTranscript, bool)> finishedCallback)
{
    ...
    auto *progressDialog = new QProgressDialog(obsText("Status.TranscribingAudio"), obsText("Button.Cancel"), 0, 100,
                                               parent ? parent : nullptr);
    progressDialog->setWindowTitle(obsText("Dialog.TranscribingAudioTitle"));
    configure_foreground_progress_window(progressDialog, parent, true);
    progressDialog->setMinimumDuration(0);
                                         ...
                                         std::function<void(RecordingTranscript, bool)> finishedCallback)
{
    ...
    if (transcribeOnDemand) {
        transcribe_video_with_progress_dialog_async(parent, videoPath, transcriptionLanguage, std::move(finishedCallback));
        return;
    }

    load_cached_transcript_async(
        parent, videoPath, transcriptionLanguage,
        [parent, videoPath, transcriptionLanguage, finishedCallback = std::move(finishedCallback)](
            RecordingTranscript transcript) mutable {
