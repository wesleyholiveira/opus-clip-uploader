...
DriveRequest::DriveRequest(std::string accessToken)
    : accessToken(std::move(accessToken))
{
}

UploadResult DriveRequest::createResumableSession(
    ...
    if (httpStatus < 200 || httpStatus >= 300) {
        return {false, false, httpStatus, response, {}, 0, 0,
            {static_cast<int>(httpStatus), "Failed to create resumable upload session. HTTP status " + std::to_string(httpStatus)}};
    }

    if (responseHeaders.location.empty()) {
        return {false, false, httpStatus, response, {}, 0, 0,
            {-1, "Google Drive did not return a resumable session Location header"}};
    }
...
UploadResult DriveRequest::queryUploadStatus(const std::string& sessionUrl, std::uint64_t totalSize)
{
    ...
    return {false, false, httpStatus, response, sessionUrl, 0, 0,
        {static_cast<int>(httpStatus), "Failed to query resumable upload status. HTTP status " + std::to_string(httpStatus)}};
}

UploadResult DriveRequest::uploadChunk(
    ...
    if (!file) {
        return {false, false, 0, {}, sessionUrl, 0, start,
            {-1, "Failed to seek file to offset " + std::to_string(start)}};
    }
    ...
    const std::string contentRange =
        "Content-Range: bytes " + std::to_string(start) + "-" + std::to_string(end) + "/" + std::to_string(totalSize);
    ...
    return {false, false, httpStatus, response, sessionUrl, 0, start,
        {static_cast<int>(httpStatus), "Upload chunk failed. HTTP status " + std::to_string(httpStatus)}};
}

UploadResult DriveRequest::uploadFileResumable(
    const std::string& path,
    const std::string& fileName,
    const std::string& mimeType,
    const std::string& folderId,
    ...
    if (!fileSizeResult.has_value()) {
        return {false, false, 0, {}, {}, 0, 0,
            {-1, "Could not determine file size: " + path}};
    }
    ...
    while (offset < fileSize) {
        UploadResult chunkResult = uploadChunk(
            uploadSessionUrl,
            path,
            offset,
            DefaultChunkSize,
